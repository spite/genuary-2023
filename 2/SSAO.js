...
void main() {
  ...
  // vec3 cx = vec3(142., 214., 178.) / 255.;
  // vec3 cy = vec3(213., 75., 73.) / 255.;
  // vec3 cz = vec3(47., 44., 94.) / 255.;

  vec3 cx = vec3(201., 156., 67.) / 255.;
  vec3 cy = vec3(228., 204., 96.) / 255.;
  vec3 cz = vec3(168., 114., 58.) / 255.;

  if(abs(dot(normal, vec3(0.,1.,0.))) > 0.) {
    vColor = cy;
  } else if (abs(dot(normal, vec3(1.,0.,0.))) > 0.) {
    vColor = cx;
  } else {
    vColor = cz;
  }
